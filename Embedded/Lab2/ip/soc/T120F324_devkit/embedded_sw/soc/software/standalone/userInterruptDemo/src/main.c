...
/******************************************************************************
*
* @file main.c: userInterruptDemo
*
* @brief This demo shows how to handle a user interrupt that accepts an interrupt signal 
*        from user logic. In this demo, ten seconds after the Sapphire SoC comes out of 
*        reset, the user interrupt gets a trigger from the external module. Operation
*        jumps from the main routine to the interrupt routine. When the interrupt code 
*        finishes executing, it jumps back to the main routine.
*
******************************************************************************/
#include <stdint.h>
#include "bsp.h"
...
/******************************************************************************
*
* @brief This main function initializes the system and then enters an infinite 
*        loop while waiting for a user interrupt.
*
******************************************************************************/
void main() {
    init();
    bsp_printf("user interrupt(s) demo, waiting for user interrupt... \r\n");
    while(1); //Idle
}

/******************************************************************************
*
* @brief This function initialize user interrupt depending on the avaibility 
*        of user interrupts based on Sapphire Soc configuration. 
*
******************************************************************************/
void init(){
...
/******************************************************************************
*
* @brief This function handles user interrupts by claiming pending interrupts 
* 		 and executing the appropriate routine based on the interrupt that was
*        claimed. 
*
******************************************************************************/
void userInterrupt(){
    ...
    while(claim = plic_claim(BSP_PLIC, BSP_PLIC_CPU_0)){
        ...
        case SYSTEM_PLIC_USER_INTERRUPT_A_INTERRUPT:
            bsp_printf("Entered User Interrupt 0 Routine \r\n");
            break;
#endif
#ifdef SYSTEM_PLIC_USER_INTERRUPT_B_INTERRUPT
        case SYSTEM_PLIC_USER_INTERRUPT_B_INTERRUPT:
            bsp_printf("Entered User Interrupt 1 Routine \r\n");
            break;
#endif
#ifdef SYSTEM_PLIC_USER_INTERRUPT_C_INTERRUPT
        case SYSTEM_PLIC_USER_INTERRUPT_C_INTERRUPT:
            bsp_printf("Entered User Interrupt 2 Routine \r\n");
            break;
#endif
#ifdef SYSTEM_PLIC_USER_INTERRUPT_D_INTERRUPT
        case SYSTEM_PLIC_USER_INTERRUPT_D_INTERRUPT:
            bsp_printf("Entered User Interrupt 3 Routine \r\n");
            break;
#endif
#ifdef SYSTEM_PLIC_USER_INTERRUPT_E_INTERRUPT
        case SYSTEM_PLIC_USER_INTERRUPT_E_INTERRUPT:
            bsp_printf("Entered User Interrupt 4 Routine \r\n");
            break;
#endif
#ifdef SYSTEM_PLIC_USER_INTERRUPT_F_INTERRUPT
        case SYSTEM_PLIC_USER_INTERRUPT_F_INTERRUPT:
            bsp_printf("Entered User Interrupt 5 Routine \r\n");
            break;
#endif
#ifdef SYSTEM_PLIC_USER_INTERRUPT_G_INTERRUPT
        case SYSTEM_PLIC_USER_INTERRUPT_G_INTERRUPT:
            bsp_printf("Entered User Interrupt 6 Routine \r\n");
            break;
#endif
#ifdef SYSTEM_PLIC_USER_INTERRUPT_H_INTERRUPT
        case SYSTEM_PLIC_USER_INTERRUPT_H_INTERRUPT:
            bsp_printf("Entered User Interrupt 7 Routine \r\n");
            break;
#endif
            default: crash(); break;
        }
